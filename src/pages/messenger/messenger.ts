...
function messengerRender(
  type: 'default' | 'tooltips' | 'modal' | 'rename'
): void {
  ...
  const topTooltip: Tooltip = new Tooltip(toptooltipData);

  const bottomTooltip: Tooltip = new Tooltip(bottomtooltipData);

  const renameForm: RenameForm = new RenameForm(renameFormData);
  ...
  const chat: Chat = new Chat(chatData);
