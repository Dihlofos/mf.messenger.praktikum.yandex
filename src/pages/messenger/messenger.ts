...
function messengerRender(type: 'default' | 'tooltips' | 'modal' | 'rename'):void {
  const root: HTMLElement | null = document.querySelector(".root");

  const searchField:Field = new Field({
      name: 'search',
      type: 'text',
      label: 'Поиск',
      value: '',
  ...
  const topTooltip: Tooltip = new Tooltip({
  ...
  const bottomTooltip: Tooltip = new Tooltip({
...
const renameForm: RenameForm = new RenameForm({
  mix: ''
})

  const currentChat:CurrentChat = new CurrentChat({
